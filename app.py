...
def get_cohort_data():
    ...
    # Positive: Mean ~0.66 (yields misses for traditional AI intentionally to show contrast)
    # Negative: Mean ~0.2
    ai_prob = np.where(
        actual_positive == 1,
        np.random.beta(a=6, b=3, size=n),  
        np.random.beta(a=2, b=8, size=n)   
    )
    
    # Realism factor 1: Hard cases where AI gets it wrong (or is borderline) generate higher uncertainty.
    ai_uncertainty = np.where(
        ((actual_positive == 1) & (ai_prob < 0.5)) | ((actual_positive == 0) & (ai_prob > 0.5)),
        np.random.uniform(0.3, 0.6, size=n), # High uncertainty for incorrect/borderline cases
        np.random.uniform(0.0, 0.2, size=n)  # Low uncertainty for clear cases
    )

    # Realism factor 2: Actual Positive cases present with higher clinical severity.
    severity = np.where(
        actual_positive == 1,
